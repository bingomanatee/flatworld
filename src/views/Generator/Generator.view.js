...
export default bottle.container.injectState(class Generator extends Component {
  ...
  zoomOut() {
    if (this.state.zoom > 0) this.setState({zoom: this.state.zoom - 1})
  }

    zoomIn() {
    if (this.state.zoom < ZOOMS.length) this.setState({zoom: this.state.zoom + 1});
  }
     ...
     <div className={style['Generator-canvas-wrapper']}>
       <div className={style['info']}>
         ...
         <div>
           ...
           <Button raised primary onClick={() => this.zoomIn()} iconBefore={false} iconChildren="navigate_next">out</Button>
         </div>
       </div>
  ...
  componentDidUpdate() {
    if (this.state.generator) this.state.generator.zoom = ZOOMS[this.state.zoom];
  }
});